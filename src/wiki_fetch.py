            ...
            try: 
                ...
                if len(category_keywords & relevant_categories) > 0:
                    ...
                    if os.path.isfile(articlefilename):
                         filename = "content_"+ str(article)+'%s.txt' % str(term+time.strftime("%Y%m%d-%H%M%S"))
...
conditions = ["heart attack","palpitations"]
for condition in conditions:
    relArticles = findRelevantArticles("Heart Attack",data_path=os.path.join('./data/wikipedia',condition))
